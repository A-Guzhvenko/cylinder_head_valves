...
home_dir = home_dir+'/linuxcnc/project/valve_head/'
#------------------------------------------------------------------------------------------------#
...
class ValveSeatContour:
    ...
    def get_breakpoints(self):
        ...
        x2_orig = -self.vsdtr / 2 + self.vsw * np.cos(vsa_rad)
        y2_orig = -self.vsw * math.sin(vsa_rad) + self.y_offset
        x2 = x2_orig
        y2 = y2_orig

        # Третья точка: максимальная глубина
        x3_orig = x2_orig + (self.vsd - self.vsw * math.sin(vsa_rad)) / math.tan(vsa2_rad)
        y3_orig = -self.vsd + self.y_offset
        x3 = x3_orig
        y3 = y3_orig if y3_orig <= 0 else 0
...
class HandlerClass:
    ...
    def file_path(self, file_name: str) -> str:
        ...
        file_path = home_dir + name_pr +'/'+ file_name
    ...
    def get_entry_value(self, entry_name, default=0.0):
        ...
        if not entry:
            self.log_message(f"Поле ввода '{entry_name}' не найдено")
            return default
        try:
            return float(entry.get_text().replace(',', '.'))
        except ValueError:
            self.log_message(f"Невозможно преобразовать значение из '{entry_name}' в число")
            return default

    def get_current_pilot(self):
        f_slow    = self.get_entry_value("f_slow")
        f_fast    = self.get_entry_value("f_fast")
        d_pilot   = self.get_entry_value("d_pilot")
        h_pilot   = self.get_entry_value("h_pilot")
        clearance = self.get_entry_value("clearance")
        return f_slow, f_fast, d_pilot, h_pilot, clearance
    ...
    def get_z(self):
        try:
            ...
            print("error", detail)
            sys.exit(1)
        return(round(s.position[2], 2))

    def create_program(self):
        # Получаем текущие значения параметров
        f_slow, f_fast, d_pilot, h_pilot, clearance = self.get_current_pilot()
        ...
        #     f'G0 X{d_pilot/2+2}\n'
        #     f'G0 Z{-h_pilot}\n'
        #     f'G38.2 X{-d_pilot/2+2} F{f_fast}\n'
        #     f'G38.4 X{clearance*1.2} F{f_slow}\n'
        #     f'G38.2 X{-clearance*3} F{f_slow}\n'
        #     f'#1 = #<_x>\n'
        #     f'G0 X{clearance*2}\n'
        #     f'G0 Z{h_pilot}\n'
        #     f'G90\n'
        ...
        #     f'G0 Y{d_pilot/2+2}\n'
        #     f'G0 Z{-h_pilot}\n'
        #     f'G38.2 Y{-d_pilot/2+2} F{f_fast}\n'
        #     f'G38.4 Y{clearance*1.2} F{f_slow}\n'
        #     f'G38.2 Y{-clearance*3} F{f_slow}\n'
        #     f'#3 = #<_y>\n'
        #     f'G0 Y{clearance*2}\n'
        #     f'G0 Z{h_pilot}\n'
        #     f'G90\n'
        ...
        programm = (
            ...
            f'G0 X{d_pilot/2+2}\n' # Быстро переместиться по X на позицию (d_pilot/2 + 2 мм) от текущей — вход в зону измерения
            f'G0 Z{-h_pilot}\n' # Быстро опуститься по Z на глубину h_pilot (вниз — отрицательное значение)
            f'G38.2 X{-d_pilot/2+2} F{f_fast}\n' # Профильное зондирование влево (по X) до срабатывания датчика (F быстрое), начиная с текущей позиции
            f'G38.4 X{clearance*1.2} F{f_slow}\n' # Отъехать назад (вправо) на 1.2×clearance с медленной подачей для точности
            f'G38.2 X{-clearance*3} F{f_slow}\n' # Повторное зондирование влево на 3×clearance с медленной подачей для точного определения края
            f'#1 = #<_x>\n' # Зафиксировать координату X левого края заготовки в переменную #1
            f'G0 X{clearance*2}\n'# Отъехать вправо на 2×clearance, чтобы освободить зону
            f'G0 Z{h_pilot}\n' # Поднять инструмент на h_pilot (вверх), завершение измерения по X слева
    ...
    def change_table(self, widget):
        ...
        # Получаем состояние радиокнопок
        valve_in_btn = self.builder.get_object("valv_in")
        valve_out_btn = self.builder.get_object("valv_out")

        # Получаем значение номера клапана
        val_num = self.get_entry_value("val_num") # читаем поле номера преобразуем в флоат
        len_tab = len(self.table_data) # находим длинну списка
        val_num_tab = int(val_num) if val_num != 0 else len_tab + 1 #устанавливаем номер клапана, если не задан, то следующий по порядку
        val_num = self.builder.get_object("val_num") 
        val_num.set_text("") # очищаем поле номера

        # Получаем состояние кривизны плоскости
        curva_koef = self.get_entry_value("curva_koef") # читаем поле коэффициента кривизны
        curva = self.builder.get_object("curva_koef") 
        curva.set_text("0.0") # очищаем поле кривизны плоскости

        # Получаем состояние примечания
        comment = self.builder.get_object("comment")
        comment_text=comment.get_text()
        comment.set_text("") # очищаем поле комента

        if valve_in_btn and valve_in_btn.get_active():
            valve_type = "впускной"
        elif valve_out_btn and valve_out_btn.get_active():
            valve_type = "выпускной"
        else:
            valve_type = "неизвестно"

        # Формируем строку и добавляем в dataset
        row = [val_num_tab, f"{self.x:.2f}", f"{self.y:.2f}", valve_type, curva_koef, comment_text]
        # Флаг, чтобы знать, была ли найдена и обновлена строка
        found = False
        # Проходим по индексам списка, чтобы можно было заменить элемент
        for i in range(len(self.table_data)):
            if int(self.table_data[i][0]) == val_num_tab:
                self.table_data[i] = row  # Заменяем существующую строку
                found = True
                break

        # Если не нашли совпадения — добавляем новую строку
        if not found:
        ...
        # Сохранить в файл
        table_temp = pd.DataFrame(self.table_data)
        table_temp.to_csv(self.file_path("table_center.csv"), index=False, header=False)
